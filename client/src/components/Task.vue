<template>
  <div class="task my-1">
        ...
        <div class="row justify-content-around">
          ...
          <div class="col-4"><img class="dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"
            ...
            <div class="dropdown-menu" aria-labelledby="btnGroupDrop1" x-placement="bottom-start" style="position: absolute; transform: translate3d(0px, 38px, 0px); top: 0px; left: 0px; will-change: transform;">
              <a v-for="list in lists" class="dropdown-item" href="#" @click="moveTask(moveTaskId, list._id)">{{list.title}}</a></div>
          </div>
        </div>
      </div>
    </div>
    <form v-if="commentInput" class="form-inline my-2 my-lg-0">
      ...
      <button class="btn boarddrop my-2 my-sm-0" style="font-family: 'Share Tech Mono', monospace; color:#1B4E5F;" type="submit"
        @click="makeComment(board, moveTaskId)">Comment</button></form>
    <div class="row my-3">
      <ul>
...
<script>
  export default {
    name: 'task',
    props: ['taskId', 'title', 'authorId', 'boardId', 'comments'],
    computed: {
      lists() {
